...
describe('generateLoanNumber', () => {
  it('should return 69-03-1 for UTC date in Bangkok March', () => {
    ...
    expect(generateLoanNumber(new Date('2026-03-15T10:00:00Z'), 1)).toBe(
      '69-03-1',
    );
  });

  it('should pad month with leading zero', () => {
    expect(generateLoanNumber(new Date('2026-01-15T10:00:00Z'), 1)).toBe(
      '69-01-1',
    );
  });

  it('should NOT pad seq (no leading zero)', () => {
    expect(generateLoanNumber(new Date('2026-03-15T10:00:00Z'), 10)).toBe(
      '69-03-10',
    );
  });
  ...
  it('should use Bangkok time for month boundary — UTC March is Bangkok April', () => {
    ...
    expect(generateLoanNumber(new Date('2026-03-31T18:00:00Z'), 1)).toBe(
      '69-04-1',
    );
  });
  ...
  it('should use Bangkok time — UTC Dec 31 late night stays December in Bangkok', () => {
    ...
    expect(generateLoanNumber(new Date('2026-12-31T10:00:00Z'), 3)).toBe(
      '69-12-3',
    );
  });
  ...
  it('should convert CE year to Buddhist Era (+543)', () => {
    // 2027 CE → 2570 BE → 70
    expect(generateLoanNumber(new Date('2027-01-15T10:00:00Z'), 1)).toBe(
      '70-01-1',
    );
  });
  ...
  it('should handle year boundary with Bangkok offset — UTC Jan 1 00:00 = Bangkok Jan 1 07:00', () => {
    ...
    expect(generateLoanNumber(new Date('2027-01-01T00:00:00Z'), 1)).toBe(
      '70-01-1',
    );
  });
});
