...
const FIRST_NAMES = [
  'สมชาย', 'สมหญิง', 'วิชัย', 'วิชชุดา', 'อนุชา',
  'อนุชิต', 'ประยุทธ์', 'ประยูร', 'นิพนธ์', 'นิตยา',
  'สุรชัย', 'สุรีรัตน์', 'ธนาคม', 'ธนาภรณ์', 'กิตติ',
  'กิตติมา', 'พิษณุ', 'ชัยวัฒน์', 'มนตรี', 'มนัสสา',
];

const LAST_NAMES = [
  'ใจดี', 'มั่นคง', 'รักดี', 'สวัสดิ์', 'พันธ์ดี',
  'บุญมา', 'ศรีสุข', 'วงศ์สา', 'แสนดี', 'ทองคำ',
  'เพ็งจันทร์', 'กุลบุตร', 'อินทรา', 'นาคา', 'มณีรัตน์',
  'สิงห์ทอง', 'ขุนทอง', 'ผาทอง', 'จันทร์หอม', 'ดวงดี',
];

const PROVINCES = [
  'กรุงเทพมหานคร', 'เชียงใหม่', 'นครราชสีมา', 'ขอนแก่น',
  'อุดรธานี', 'สุราษฎร์ธานี', 'ภูเก็ต', 'ชลบุรี', 'นครปฐม', 'อยุธยา',
];

const PLATE_PREFIXES = [
  'กข', 'กค', 'ขข', 'ขค', 'งง', 'จจ', 'ชช', 'ซซ', 'ญญ', 'ฐฐ',
];

const VEHICLE_MODELS = [
  'Honda Wave 125', 'Yamaha Fino', 'Honda Click 125', 'Suzuki Raider',
  'Honda PCX 160', 'Yamaha NMAX', 'Honda Scoopy', 'Honda Airblade',
  'Kawasaki D-Tracker', 'Yamaha Aerox',
];
...
const OCCUPATIONS = [
  'ค้าขาย', 'รับจ้าง', 'เกษตรกร', 'พนักงานบริษัท', 'ข้าราชการ',
  'ประกอบธุรกิจส่วนตัว', 'ช่างซ่อมรถ', 'ขับรถรับจ้าง', 'พนักงานโรงงาน', 'แม่บ้าน',
];
...
async function main() {
  ...
  console.log(`🌱 Seeded ${TOTAL} loans (IDs: ${created[0]}–${created[TOTAL - 1]})`);
  console.log('\nSearch test samples:');
  ...
  console.log(`  loanNumber → "${generateLoanNumber(now, existing + 1)}" (first seeded loan)`);
